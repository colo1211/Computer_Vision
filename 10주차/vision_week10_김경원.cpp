...
int main()
{
	...
	// (50,40) 위치를 특정 색상으로 
	// 트루컬러 영상 -> 가로*3 세로*3, B(0)G(1)R(2) 순서 

	//Image[40 * W *3 + 50 * 3] = 128;  // Blue 성분  
	//Image[40 * W *3 + 50 * 3 + 1] = 128;  // Green 성분 
	//Image[40 * W *3 + 50 * 3 + 2] = 128;  // Red 성분
	...
	// (50, 100) ~ (300, 400) 박스 채우기(사각형 색 채우기) 
	...
	// y좌표 기준 120 ~ 359 (Green) +1
	for (int i = 120; i < 360; i++) {
		for (int j = 0; j < W; j++) {
			Image[i * W * 3 + j * 3 + 1] = 255;
		}
	}
	// y좌표 기준 240 ~ 479 (Blue) +0 
	...
	*/
	...
	// 그라데이션 만들기 (Blue ~ Red) , 점진적으로 변환하기 
	//double wt;
	//for(int a = 0; a<120; a++){
	//	for (int i = 0; i < W; i++) {
	//		wt = i / (double)(W - 1); // 0~1 까지 연동 
	...
	// 10주차 과제 
	double wt;
	for (int a = 0; a < 160; a++) { // 영상의 아랫 부분 
		for (int i = 0; i < W; i++) {
			wt = i / (double)(W - 1);
			Image[a * W * 3 + i * 3 + 0] = (BYTE)(255 * (1.0 - wt));//Blue에서 시작 
			Image[a * W * 3 + i * 3 + 1] = (BYTE)(255 * wt);//Green 
			Image[a * W * 3 + i * 3 + 2] = (BYTE)(255 * wt);//Red
			//끝으로 가서 Green과 Red 가 255가 되어서 Yellow 가 출력 
		}
	}
	for (int a = 160; a < 320; a++) { // 영상의 중간 부분 
		for (int i = 0; i < W; i++) {
			wt = i / (double)(W - 1);
			Image[a * W * 3 + i * 3 + 0] = (BYTE)(255 * wt); //Blue
			Image[a * W * 3 + i * 3 + 1] = (BYTE)(255 * (1.0 - wt)); //Green에서 시작
			Image[a * W * 3 + i * 3 + 2] = (BYTE)(255 * wt); //Red
			// 끝으로 가서 Blue와 Red 가 255가 되어서 Magenta(자주색)가 출력 
		}
	}
	for (int a = 320; a < 480; a++) { // 영상의 윗 부분 
		for (int i = 0; i < W; i++) {
			wt = i / (double)(W - 1);
			Image[a * W * 3 + i * 3 + 0] = (BYTE)(255 * wt);  //Blue 
			Image[a * W * 3 + i * 3 + 1] = (BYTE)(255 * wt); //Green
			Image[a * W * 3 + i * 3 + 2] = (BYTE)(255 * (1.0 - wt)); //Red에서 시작
			// 끝으로 가서 Green과 Blue 가 255가 되어서 Cyan(청록색)이 출력
		}
	}
