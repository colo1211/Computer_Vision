...
void DrawFaceRectangle(BYTE * Output, int W, int H, int LU_X, int LU_Y, int RD_X, int RD_Y) {
	// 위쪽 변
	for (int j = RD_X; j < LU_X; j++) {
		Output[RD_Y * W * 3 + j * 3 + 2] = 255;
		Output[RD_Y * W * 3 + j * 3 + 0] = Output[RD_Y * W * 3 + j * 3 + 1] = 0;
	}
	// 아래쪽 변
	for (int j = RD_X; j < LU_X; j++) {
		Output[LU_Y * W * 3 + j * 3 + 2] = 255;
		Output[LU_Y * W * 3 + j * 3 + 0] = Output[LU_Y * W * 3 + j * 3 + 1] = 0;
	}
	// 왼쪽 변
	for (int i = RD_Y; i <= LU_Y; i++) {
		Output[i * W * 3 + RD_X * 3 + 2] = 255;
		Output[i * W * 3 + RD_X * 3 + 0] = Output[i * W * 3 + RD_X * 3 + 1] = 0;
	}
	// 오른쪽 변
	for (int i = RD_Y; i <= LU_Y; i++) {
		Output[i * W * 3 + LU_X * 3 + 2] = 255;
		Output[i * W * 3 + LU_X * 3 + 0] = Output[i * W * 3 + LU_X * 3 + 1] = 0;
	}
}
...
int main()
{
	...
	*/
	...
	//			Output[i * W * 3 + j * 3] = Image[i * W * 3 + j * 3 + 0]; // B
	...
	//}
		...
		for (int i = 0; i < H; i++) {
			for (int j = 0; j < W; j++) {
				if (Cb[i * W + j] > 85 && Cb[i * W + j] < 125 && Cr[i * W + j]>135 && Cr[i * W + j] < 185 && Y[i * W + j]>80) {
					Output[i * W * 3 + j * 3 + 0] = Image[i * W * 3 + j * 3 + 0]; //B
					Output[i * W * 3 + j * 3 + 1] = Image[i * W * 3 + j * 3 + 1];//G
					Output[i * W * 3 + j * 3 + 2] = Image[i * W * 3 + j * 3 + 2];//R
				}
				else Output[i * W * 3 + j * 3 + 0] = Output[i * W * 3 + j * 3 + 1] = Output[i * W * 3 + j * 3 + 2] = 0;
			}
		}
		
		int LU_X, LU_Y, RD_X, RD_Y=0;
		...
		for (int i = 0; i < H; i++)//y
		{
			for (int j = 0; j < W; j++)//x
			{
				if (Output[i * W *3 + j*3+0] != 0 && Output[i * W * 3 + j * 3+1] != 0 && Output[i * W * 3 + j * 3+2] != 0)
				{
					LU_Y = i;
					break;
				}
		...
		printf("LU_X: %d, LU_Y: %d, RD_X: %d, RD_Y: %d", LU_X, LU_Y, RD_X, RD_Y);

		DrawFaceRectangle(Output, W, H, LU_X, LU_Y, RD_X, RD_Y);
