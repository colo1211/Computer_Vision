...
// 침식, 붙어있는 것들을 떨어뜨릴수 있지만, 전경(사물)의 크기또한 줄어든다. 
		...
		for (int j = 1; j < W - 1; j++) {
			if (Image[i * W + j] == 255) // 전경화소라면
			{
				if (!(Image[(i - 1) * W + j] == 255 &&
					Image[(i + 1) * W + j] == 255 &&
					Image[i * W + j - 1] == 255 &&
					Image[i * W + j + 1] == 255)) // 4주변화소가 모두 전경화소가 하나라도 아니라면
					Output[i * W + j] = 0;
				else Output[i * W + j] = 255;
			}
			else Output[i * W + j] = 0; // 전경화소가 아니라면
		}
	}
}
...
void FeatureExtractThinImage(BYTE* Image, BYTE* Output, int W, int H) {
	...
	for (int i = 1; i < H - 1; i++) {
		for (int j = 1; j < W - 1; j++) {
			if (Image[i * W + j] == 0) // 전경 - 흑, 배경 - 백 
			{
				int k = 0;
				while (k <= 7) {
					int ny = i + dy[k];
					int nx = j + dx[k];
					if (Image[ny * W + nx] == 0) count++;
					k++;
				}
				if (count == 1 || count >= 3) {
					for (int z = 0; z < 8; z++) { // 분기점, 끝점 역시 8방향으로 찍었기에 두껍게 검출됨을 확인할 수 있다.
						int ny_2 = i + dy[z];
						int nx_2 = j + dx[z];
						Output[ny_2 * W + nx_2] = 100;
					}
				}
				count = 0;
			}
...
int main()
{
	...
	*/
	...
	// 팽창 3번을 통해 머리카락같던 요소들 제거 후 뚱뚱해진 글씨들을 
	// 침식을 통해 글씨 사이즈를 작게함
	// Closing
